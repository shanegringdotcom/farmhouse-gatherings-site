import { handleOwnerRez } from "./ownerrez";

// Types for the bindings come from worker-configuration.d.ts (generated by
// `wrangler types`, pulled in via worker/tsconfig.json).
...
declare global {
  interface Env {
    ...
    // OwnerRez direct booking. USERNAME is the OwnerRez login email and PAT the
    // `pt_...` token from app.ownerrez.com/settings/api — both via
    // `wrangler secret put`, never in wrangler.jsonc or .env (Vite inlines
    // anything it can see into the client bundle). PROPERTY_ID is a plain var.
    OWNERREZ_USERNAME?: string;
    OWNERREZ_PAT?: string;
    OWNERREZ_PROPERTY_ID?: string;
  }
}
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    // OwnerRez direct booking: availability, live pricing, and the hand-off to
    // OwnerRez's hosted payment + rental-agreement form. All of it is
    // server-side because the Personal Access Token lives here and nowhere
    // else. Already covered by assets.run_worker_first's /api/* glob.
    if (url.pathname === "/api/ownerrez" || url.pathname.startsWith("/api/ownerrez/")) {
      return handleOwnerRez(request, env, url);
    }
